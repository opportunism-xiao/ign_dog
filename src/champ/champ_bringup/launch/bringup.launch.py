...
def generate_launch_description():
    ...
    declare_use_foot_contacts = DeclareLaunchArgument(
        "use_foot_contacts",
        default_value="true",
        description="Use foot_contacts topic in state estimation",
    )
    ...
    declare_use_base_to_footprint_ekf = DeclareLaunchArgument(
        "use_base_to_footprint_ekf",
        default_value="false",
        description="Enable base_to_footprint EKF (publishes base_footprint)",
    )

    declare_use_footprint_to_odom_ekf = DeclareLaunchArgument(
        "use_footprint_to_odom_ekf",
        default_value="false",
        description="Enable footprint_to_odom EKF (publishes odom)",
    )
    ...
    state_estimator_node = Node(
        ...
        parameters=[
            ...
            {"use_foot_contacts": LaunchConfiguration("use_foot_contacts")},
            {"urdf": ParameterValue(Command(['xacro ', LaunchConfiguration('description_path')]), value_type=str)},
            LaunchConfiguration('joints_map_path'),
            LaunchConfiguration('links_map_path'),
            LaunchConfiguration('gait_config_path'),
    ...
    base_to_footprint_ekf = Node(
        ...
        output="screen",
        condition=IfCondition(LaunchConfiguration("use_base_to_footprint_ekf")),
        parameters=[
    ...
    footprint_to_odom_ekf = Node(
        ...
        output="screen",
        condition=IfCondition(LaunchConfiguration("use_footprint_to_odom_ekf")),
        parameters=[
            {"base_link_frame": LaunchConfiguration("base_link_frame")},
            {"use_sim_time": LaunchConfiguration("use_sim_time")},
            os.path.join(
    ...
    return LaunchDescription(
        [
            ...
            declare_use_foot_contacts,
            declare_publish_odom_tf,
            declare_use_base_to_footprint_ekf,
            declare_use_footprint_to_odom_ekf,
